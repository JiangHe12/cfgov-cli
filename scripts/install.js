...
const EXPECTED_BINARIES = Object.freeze([
  'cfgov-cli-darwin-amd64',
  'cfgov-cli-darwin-arm64',
  'cfgov-cli-linux-amd64',
  'cfgov-cli-linux-arm64',
  'cfgov-cli-windows-amd64.exe',
  'cfgov-cli-windows-arm64.exe',
]);
...
function applyMirror(canonicalUrl) {
  const mirror = envWithDeprecatedAlias('CFGOV_DOWNLOAD_MIRROR', 'CFGOV_CLI_DOWNLOAD_MIRROR');
...
async function main() {
  ...
  const destination = path.join(destDir, os === 'windows' ? 'cfgov.exe' : 'cfgov');

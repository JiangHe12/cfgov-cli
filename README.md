...
Nacos and Apollo report `supportsCas=false`. Rule and feature-flag reads remain
available, but every rule/flag blob write requires an atomic precondition:
absence for initial creation and a revision for an existing blob. Those writes
therefore return `NOT_IMPLEMENTED` on Nacos and Apollo. The capability fields
`supportsRuleWrites` / `supportsFlagWrites` expose the overall boundary; the
existing-resource fields remain available for compatibility.

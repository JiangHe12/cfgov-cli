...
```bash
# 1. 把 cfgov 指向你的配置中心(保存为可复用的「上下文 context」)
...
cfgov audit prune  (--before <…> | --keep-last <n>) [--confirm]                       # 不加 --confirm 即 dry-run
...
> `backup clean` 和 `audit prune` **只删本地文件**,默认 **dry-run**,必须加 `--confirm` 才真正删除。删除动作本身也会被审计。
</details>

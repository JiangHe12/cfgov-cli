...
func verifyMutationSpoolDirectory(path string) error {
	return verifyMutationSpoolDirectoryPath(path)
}

func verifyMutationSpoolDirectoryPath(path string) error {
...
func verifyMutationSpoolParent(path string) error { //nolint:gocyclo // Windows DACL validation must inspect every ACE and reject unsafe parent replacement rights.
	info, err := os.Lstat(path)
	if err != nil {
	...
	dangerous := // Keep this list explicit: any one right permits spool modification or replacement.
	...
	for index := uint16(0); index < dacl.AceCount; index++ {
		...
		if ace.Header.AceType != windows.ACCESS_ALLOWED_ACE_TYPE {
			return apperrors.New(apperrors.CodeLocalIOError, "mutation outcome spool parent has an unexpected ACE type", nil)
		}
		sid := (*windows.SID)(unsafe.Pointer(&ace.SidStart)) //nolint:gosec // The SID is stored in the documented ACE tail.
		...
		if ace.Mask&dangerous != 0 {
			return apperrors.New(
				...
				"mutation outcome spool parent grants write or replacement rights to an untrusted SID",
				nil,
			)
		}
	}
...
func verifyMutationSpoolACL(path string) error { //nolint:gocyclo // Exact owner/SYSTEM/Administrators ACL validation intentionally checks every ACE.
	userSID, systemSID, adminSID, err := trustedMutationSpoolSIDs()
	if err != nil {
		return err
	}
...
func syncMutationSpoolDirectory(_ string) error {
	// The committed file uses MOVEFILE_WRITE_THROUGH. If a replayed delete is
	// observed again after a crash, replay is intentionally at-least-once and
	// deduplicated by mutationId + phase.
	return nil
}
